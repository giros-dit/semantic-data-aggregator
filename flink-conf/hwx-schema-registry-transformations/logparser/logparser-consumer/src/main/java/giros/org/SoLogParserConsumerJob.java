...
public class SoLogParserConsumerJob {
	public static void main(String[] args) throws Exception {
		...
		KafkaDeserializationSchema<SoInstantiationMetrics> schema = ClouderaRegistryKafkaDeserializationSchema
				.builder(SoInstantiationMetrics.class)/* .setRegistryAddress(schemaRegistryAddress) */
				.setConfig(schemaRegistryConf).build();

		FlinkKafkaConsumer<SoInstantiationMetrics> kafkaSource = new FlinkKafkaConsumer<>("so-instantiation-metrics", schema, props);
		...
		FlinkKafkaProducer<String> kafkaSink = new FlinkKafkaProducer<>("raw-so-instantiation-metrics", new SimpleStringSchema(), props);
