...
Let’s assume the private insecure registry is available on the Kubernetes cluster at endpoint reachable via `<k8s-registry>` (which should be the cluster address) and port `5000`. The images we build need to be tagged with the registry endpoint before pushing them:
