...
```bash
kubectl apply -f flink-cluster-templates/test-services/zookeeper.yaml
kubectl apply -f flink-cluster-templates/test-services/kafka.yaml
kubectl apply -f flink-cluster-templates/test-services/goflow2.yaml
```
...
Now, we can deploy the [Flink cluster with the NetFlow driver job](../flink-operator/flink-cluster-templates/netflow/source-driver/json/flinkoperator-flinkjobcluster-netflow.yaml) as a custom resource in Kubernetes with:

```bash
kubectl apply -f flink-cluster-templates/netflow/source-driver/json/flinkoperator-flinkjobcluster-netflow.yaml
```
