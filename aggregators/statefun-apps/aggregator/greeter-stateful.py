...
################################################################################
# An example of stateful functions where state is being kept in Protobuf
# variable "SeenCount"
...
@functions.bind("normfuncs/greeter")
def greet(context, greet_request: GreetRequest):
    print("REQUEST ARRIVED") # TEMP***
    print("type:", type(greet_request)) # TEMP***
    print("printed:", greet_request) # TEMP***
    ...
    egress_message = kafka_egress_record(topic="normalized", key=greet_request.name, value=response)
    context.pack_and_send_egress("tidtopics/normalized", egress_message)
