...
public class YangNormalization {
     ...
     * @param interfaces_throughputKpi
     * @return
     */
    public static String serialize2JSONstring(Interfaces interfaces_throughputKpi){
		InstanceIdentifier<Interfaces> iid = InstanceIdentifier.create(Interfaces.class);
		JsonObject json_ietf_obj = new JsonObject();

		try {
			Entry<YangInstanceIdentifier, NormalizedNode<?, ?>> normalized = op_if_notification_agg_codec.toNormalizedNode(iid, interfaces_throughputKpi);
